...
export default function TableWorkspace({ token, isDark, currentDb, initialDb, initialTable, onExecute }: Props) {
  ...
  const headerHover = isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-200'
  const rowHoverBg = isDark ? 'hover:bg-white/[0.03]' : 'hover:bg-slate-50'
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      {!loading && tableName && (
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Column headers */}
          <div className={`flex shrink-0 border-b ${border} ${isDark ? 'bg-slate-800/50' : 'bg-slate-100'}`}>
            {/* Row number header */}
            <div className={`w-10 shrink-0 px-2 py-2 border-r ${border}`} />

            {columns.map((col, idx) => (
              <div
                key={`${col.name}-${idx}`}
                className={`group relative flex-1 min-w-[100px] max-w-[250px] px-3 py-2 border-r ${border} transition-colors ${headerHover} cursor-context-menu select-none`}
                onContextMenu={(e) => handleCtxMenu(e, idx)}
              >
                {modifyCol?.index === idx ? (
                  <div className="flex flex-col gap-1">
                    <input
                      type="text" value={modifyCol.name}
                      onChange={(e) => setModifyCol({ ...modifyCol, name: e.target.value })}
                      className={inputClass} autoFocus onKeyDown={(e) => { if (e.key === 'Enter') saveModifyCol(); if (e.key === 'Escape') setModifyCol(null) }}
                    />
                  ...
                  <>
                    ...
                    {col.constraints && (
                      <div className={`text-[10px] ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>{col.constraints}</div>
                    )}
                    {/* Quick modify button */}
                    <button
            ...
            {/* Add column form */}
            {showAddCol && (
              <div className={`flex items-start gap-1 px-2 py-2 min-w-[180px] border-r ${border} ${isDark ? 'bg-indigo-500/5' : 'bg-indigo-50'}`}>
            ...
            {/* Spacer matching the delete button column in data rows */}
            <div className="w-8 shrink-0 border-r border-transparent" />
            {/* Spacer to fill remaining width */}
            <div className="flex-1 border-r border-transparent" />
          </div>

          {/* Data rows */}
          <div className="flex-1 overflow-auto">
            {rows.length === 0 && !addingRow ? (
              <div className={`text-center py-16 text-xs ${muted}`}>
                No data rows
              </div>
            ) : (
              <>
                {rows.map((row, rowIdx) => (
                  <div
                    key={rowIdx}
                    className={`flex border-b transition-colors ${border} ${rowHoverBg} group`}
                  >
                    {/* Row number */}
                    <div className={`w-10 shrink-0 flex items-center justify-end px-2 py-1.5 text-[10px] ${muted} border-r ${border}`}>
                      {rowIdx + 1}
                    </div>
                    ...
                    {row.map((cell, colIdx) => {
                      ...
                      return (
                        <div
                          key={colIdx}
                          className={`flex-1 min-w-[100px] max-w-[250px] px-3 py-1.5 border-r ${border} text-xs ${
                        ...
                        >
                          {isEdit ? (
                            ...
                            <span className="cursor-pointer hover:bg-white/5 rounded px-1 -mx-1 block truncate">
                    ...
                    <div className="shrink-0 flex items-center px-1">
                      <button
                        ...
                        className={`p-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity ${
                          isDark ? 'hover:bg-red-500/10 text-slate-600 hover:text-red-400' : 'hover:bg-red-50 text-slate-300 hover:text-red-500'
                        }`}
                        title="Delete row"
                      >
                ...
                {/* Add row */}
                {addingRow && (
                  <div className={`flex border-b ${border} ${isDark ? 'bg-indigo-500/5' : 'bg-indigo-50'}`}>
                    <div className={`w-10 shrink-0 flex items-center justify-end px-2 py-1.5 text-[10px] ${muted} border-r ${border}`}>+</div>
                    {headers.map((_, colIdx) => (
                      <div key={colIdx} className={`flex-1 min-w-[100px] max-w-[250px] px-3 py-1.5 border-r ${border}`}>
                        <input
                          type="text" value={newRow[colIdx] ?? ''}
                          onChange={(e) => setNewRow((prev) => { const n = [...prev]; n[colIdx] = e.target.value; return n })}
                          placeholder="value"
            ...
            {!addingRow && tableName && (
              <button
                onClick={startAddRow}
                className={`w-full flex items-center justify-center gap-1.5 py-2 text-xs transition-colors border-b ${border} ${
